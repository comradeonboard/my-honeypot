...
def classify(event):
    ...
    try:
        ...
        et = event.get('event_type', '').upper()
        features = features + [
            1 if et == 'CONNECTION' else 0,
            1 if et == 'DISCONNECTION' else 0,
            1 if et == 'REQUEST' else 0,
        ]
        X = np.array(features).reshape(1, -1)
        X_scaled = _scaler.transform(X)
        pred_enc = int(_model.predict(X_scaled)[0])
        label = int(_encoder.inverse_transform([pred_enc])[0]) if _encoder is not None else pred_enc
        try:
